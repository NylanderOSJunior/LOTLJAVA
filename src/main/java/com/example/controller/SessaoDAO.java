...
public class SessaoDAO {
    ...
    // Método para encerrar uma sessão do banco de dados
    public boolean encerrarSessao(int sid, int serial) {
        String sql = "ALTER SYSTEM KILL SESSION ?, ?"; // Comando para matar a sessão
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, sid); // SID da sessão
            stmt.setInt(2, serial); // SERIAL da sessão

            // Executa a instrução SQL para encerrar a sessão
    ...
    public String getMinLastAnalyzedTables() {
        ...
        String sql = "SELECT MIN(last_analyzed) AS min_analyzed FROM dba_tables WHERE global_stats <>'NO' and owner = ?";
    ...
    public String getMinLastAnalyzedIndexes() {
        String schema = getCurrentSchema();
        String minLastAnalyzed = "Desconhecido";
        String sql = "SELECT MIN(last_analyzed) AS min_analyzed FROM dba_indexes WHERE global_stats <>'NO' and owner = ?";
    ...
    // Método para obter a indices fragmentados no schema do banco de dados
    ...
    public ObservableList<Sessao> buscarSessoes(String filtro) {
        ...
        String sql = "SELECT S.SID, S.SERIAL#, NVL(S.SQL_ID, 'N/A') AS SQL_ID, S.USERNAME, S.OSUSER, S.PROGRAM, S.STATUS, S.MACHINE, "
                +
...
/*
 * Para Implementar
 * SELECT TABLESPACE_NAME, FILE_NAME, BYTES/1024/1024 AS TAMANHO_MB FROM
 * DBA_DATA_FILES; --Consumo de tablespace
 * SELECT TABLESPACE_NAME, FREE_SPACE FROM DBA_FREE_SPACE; --Espaço livre
 * SELECT USERNAME, ACCOUNT_STATUS, CREATED FROM DBA_USERS; -- User Criado e
 * liberado.
 * SELECT OWNER,JOB_NAME, STATE, LAST_START_DATE, NEXT_RUN_DATE FROM
 * DBA_SCHEDULER_JOBS; --Jobs Criadas
 */