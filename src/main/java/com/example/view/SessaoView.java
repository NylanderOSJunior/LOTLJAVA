...
public class SessaoView extends Application {
    ...
    private Button btnDiagnostico = new Button("Diagnostico de Banco");
    ...
    public void start(Stage primaryStage) {
        ...
        TableColumn<Sessao, Integer> colSeria = new TableColumn<>("SERIAL");
        colSeria.setCellValueFactory(new PropertyValueFactory<>("serial"));
        ...
        tableView.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue != null) {
                ...
                int sid = newValue.getSid(); // Obtém o SID da sessão
                int serial = newValue.getSerial(); // Obtém o Serial da sessão
                abrirTelaComandoSQL(sid, serial, sqlText); // Passa o SID, Serial e o SQL para a tela de comando SQL
            }
        });
        ...
        HBox searchBox = new HBox(10, searchField, spinnerTempo, btnAplicarTempo, btnAuditoria, btnDiagnostico);
        ...
        VBox vbox = new VBox(searchBox, searchBox3, tableView, labelLock, tableBlock, pieChart, versaoLabel,
                estatisticatabLabel, estatisticaindLabel, cpuLabel, consumoLabel, searchBox2); // Adiciona o Label de
                                                                                               // versão
        vbox.setSpacing(10);
    ...
    private void abrirTelaComandoSQL(int sid, int serial, String sqlText) {
        ComandoSQLView comandoSQLView = new ComandoSQLView(sid, serial, sqlText);
        Stage stage = new Stage();
    ...
    private void exportarParaCSV() {
        ...
        if (file != null) {
            try (FileWriter writer = new FileWriter(file)) {
                writer.append("SID,SERIAL,SQL_ID,Schema,Usuário,Modulo,Status,Máquina,DuracaoConsult,Comando\n");
                for (Sessao sessao : tableView.getItems()) {
                    writer.append(sessao.getSid() + "," + sessao.getSerial() + "," + sessao.getSql_id() + ","
